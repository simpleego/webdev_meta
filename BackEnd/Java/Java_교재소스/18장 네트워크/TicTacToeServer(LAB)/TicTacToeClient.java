...
public class TicTacToeClient extends Thread {
	private JButton[][] buttons = new JButton[3][3];	// 버튼들의 배열로 보드를 표현한다. 
	...
	public void run() {
		...
		try {
			...
			if (response.startsWith("START")) {		// START 명령어이면 경기를 시작한다. 
			...
			while (true) {
				...
				} else if (response.startsWith("PRINT")) {		// 메시지를 화면에 출력한다. 
